...
public class IOSocket {
	...
	public void connect() throws IOException {
		...
		connecting = true;
		webSocket = new IOWebSocket(URI.create(webSocketAddress
				+ "/socket.io/1/websocket/" + sessionID), this, callback);
		webSocket.setNamespace(namespace);
		webSocket.connect();
	}

	public void emit(String event, String message) throws IOException {
		IOMessage packet = new IOMessage(IOMessage.EVENT, "", String.format(
				"{\"args\":\"%s\",\"name\":\"%s\"}", message, event));
	...
	public void emit(String event, JSONObject... message) throws IOException {
		try {
			...
			data.put("name", event);
			data.put("args", args);
			IOMessage packet = new IOMessage(IOMessage.EVENT, "",
					data.toString());
			webSocket.sendMessage(packet);
